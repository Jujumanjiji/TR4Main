...
#define AHMET_WALK_ANGLE ANGLE(5)
#define AHMET_RUN_ANGLE ANGLE(8)
#define AHMET_VIEW_ANGLE ANGLE(45)
#define AHMET_ENEMY_ANGLE ANGLE(90)
...
void TriggerAhmetDeathEffect(ITEM_INFO* item)
{
    if (!(wibble & 7))
    {
        SPHERE* sphere, slist[33];
        int meshCount;
        
        // cant be FALSE here because else it will be local space not world
        // because of that it cant be GetJointAbsPosition() !
        meshCount = GetSpheres(item, slist, TRUE);
        sphere = &slist[(wibble >> 3) & 1];
    ...
    TriggerDynamic(item->pos.xPos, (item->pos.yPos - STEP_L), item->pos.zPos, 13, (GetRandomControl() & 0x3F) - 64, (GetRandomControl() & 0x1F) + 96, 0);
...
#ifdef DLL_INJECT
void injector::f_game::inject_ahmet()
{
    inject(0x00401000, InitialiseAhmet);
    inject(0x00401080, AhmetControl);
    inject(0x00401730, TriggerAhmetDeathEffect);
}
#endif
