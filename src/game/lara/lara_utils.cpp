...
BOOL LaraHangTest(ITEM_INFO* item, COLL_INFO* coll)
{
    ...
    if (lara.climb_status)
    {
        ...
        {
            if (item->anim_number != ANIMATION_LARA_LADDER_TO_HANDS_LEFT && item->anim_number != ANIMATION_LARA_LADDER_TO_HANDS_RIGHT)
            {
        ...
        else if (item->anim_number == ANIMATION_LARA_HANG_IDLE && item->frame_number == (anims[ANIMATION_LARA_HANG_IDLE].frame_base + 21) && LaraTestClimbStance(item, coll))
...
{
    if (item->anim_number != ANIMATION_LARA_HANG_IDLE)
        return 0;
    if (coll->hit_static)
        return 0;
...
{
    if (item->anim_number != ANIMATION_LARA_LADDER_LEFT)
        return 0;
...
void JumpOffRope(ITEM_INFO* item)
{
    ...
    short frame = (item->frame_number - anims[ANIMATION_LARA_ROPE_SWING_FORWARD_SEMIHARD].frame_base);
    if (frame > 21)
...
void AnimateLara(ITEM_INFO* item)
{
    ...
    {
        speed = anim->yspeed + anim->yfallspeed * (item->frame_number - anim->frame_base - 1);
    ...
    else
    {
        speed = anim->yspeed;
        if (anim->yfallspeed)
            speed += (anim->yfallspeed * (item->frame_number - anim->frame_base));
        item->speed = (short)(speed >> 16);
    }
...
{
    this->inject(0x00421620, LaraFloorFront);
    this->inject(0x00420A80, LaraCeilingFront);
    this->inject(0x00422390, LaraHitCeiling);
    this->inject(0x004230E0, LaraHangTest);
    this->inject(0x00422400, LaraDeflectEdge);
    this->inject(0x00421880, LaraDeflectEdgeDuck);
    this->inject(0x00422C50, LaraDeflectEdgeJump);
    this->inject(0x00420FE0, LaraFallen);
    this->inject(0x00426600, IsValidHangPos);
    this->inject(0x00426230, LaraHangRightCornerTest);
    this->inject(0x004266E0, LaraHangLeftCornerTest);
    this->inject(0x0042CEE0, LaraClimbLeftCornerTest);
    this->inject(0x0042D160, LaraClimbRightCornerTest);
    this->inject(0x004259C0, LaraSlideEdgeJump);
    this->inject(0x00422810, LaraTestClimbStance);
    this->inject(0x00421DE0, LaraTestEdgeCatch);
    this->inject(0x00428470, LaraSlideSlope);
    this->inject(0x00428B20, LaraCollideJump);
    this->inject(0x0042C980, LaraCheckForLetGo);
    this->inject(0x0042CA60, LaraTestClimbUpPos);
    this->inject(0x0042C3C0, LaraTestClimbPos);
    this->inject(0x0042C470, LaraTestClimb);
    this->inject(0x0042CD20, LaraDoClimbLeftRight);
    this->inject(0x00431E90, LaraTestWaterClimbOut);
    this->inject(0x004321B0, LaraTestWaterStepOut);
    this->inject(0x00432A30, LaraTestWaterDepth);
    this->inject(0x00422BE0, LaraLandedBad);
    ...
    this->inject(0x004237B0, TestMonkeyRight);
    this->inject(0x00421FF0, TestHangSwingIn);
    this->inject(0x004228D0, TestWall);
    this->inject(0x00432690, SwimTurn);
    this->inject(0x00422E50, MonkeySwingFall);
    this->inject(0x00423660, MonkeySwingSnap);
    this->inject(0x004241D0, FallFromRope);
    this->inject(0x00424320, UpdateRopeSwing);
    this->inject(0x00424150, ApplyVelocityToRope);
    this->inject(0x004597D0, SetPendulumVelocity);
    this->inject(0x00426C50, SetCornerAnim);
    this->inject(0x00426AB0, CanLaraHangSideways);
    this->inject(0x00423780, GetDirOctant);
    this->inject(0x0042D7D0, GetClimbTrigger);
